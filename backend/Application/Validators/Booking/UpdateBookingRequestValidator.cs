...
public class UpdateBookingRequestValidator : AbstractValidator<UpdateBookingRequest>
{
    ...
    {
        // For update, all fields are optional, but if provided they should be validated
        ...
        RuleFor(x => x.Provider)
            .MaximumLength(200).When(x => !string.IsNullOrEmpty(x.Provider))
            .WithMessage("Provider must not exceed 200 characters.");

        RuleFor(x => x.ConfirmationCode)
            .MaximumLength(100).When(x => !string.IsNullOrEmpty(x.ConfirmationCode))
            .WithMessage("Confirmation code must not exceed 100 characters.");
        ...
        RuleFor(x => x.ContactInfo)
            .MaximumLength(500).When(x => !string.IsNullOrEmpty(x.ContactInfo))
            .WithMessage("Contact information must not exceed 500 characters.");
    }
}